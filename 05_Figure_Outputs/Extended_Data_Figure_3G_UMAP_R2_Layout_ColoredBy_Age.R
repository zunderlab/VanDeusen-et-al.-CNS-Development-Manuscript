...
print("Start UMAP_R2_Layout_ColoredBy_Age.R")
...
# Assign column names
colnames(plot_df) <- c("group","umap_x", "umap_y", "md_level")
...
# save plot for each group
groups <- sort(unique(plot_df$group))

for (g in groups) {
  plot_df_group <- filter(plot_df, group == g)
  p <- ggplot(data=plot_df_group, aes(x=umap_x, y=umap_y,
  ...
  output_filename <- paste0("Group","_", g, "_", OUTPUT_FILENAME)
  ggsave(output_filename, p, device = OUTPUT_FORMAT,
         height = OUTPUT_HEIGHT, width = OUTPUT_WIDTH)
  
}

print("Finish UMAP_R2_Layout_ColoredBy_Age.R")
