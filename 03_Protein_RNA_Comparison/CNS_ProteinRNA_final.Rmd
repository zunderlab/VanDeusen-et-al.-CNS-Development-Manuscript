---
title: "DRG Comparison From Concat Transform Files"
output: html_notebook
---
#Outline:
Section 1:
1. Get Marker names from fcs file or panel file if pipeline scripts were used
...
4. Use ensemble ids to get rna gene names from biomart... pay attn. to human vs. mouse (seem to be same for cns panel)

Section 2:
5. Get reasonable clusters for each dataset
6. Use those gene names to subset RNA data (likely have to capitalize or lowercase both to do string matching)
  - "/project/zunderlab/sarah_data/project_Protein_RNA/data/cerebellum_e11_e17/rna_preprocess_clust_layout.R"
7. Same for mass cyt, use overlapping antibody names to subset data
8. Summarize marker rexpression accross clusters for each of common markers
  - (This is the data that then can be fed into various visualizations)
...
PARENT_PATH <- "/project/zunderlab/sarah_data/project_Protein_RNA/data/cns_protein_rna_comparison" #getwd()
...
# Prep each dataset for analyses based on summarized data 
